<template>
    <div class="card mb-3 text-center mt-5 mx-auto shadow-lg custom-tam">
      ...
      <div id="SubmitOk" class="alert alert-success alert-dimissible fade show" role="alert" hidden >Usuario Registrado</div>
      <div id="SubmitFail" class="alert alert-danger" role="alert" hidden >No se puede registrar</div>  
    </div>
  </template>
    ...
    methods: {
      ...
      submitForm() {
        ...
        if (!this.v$.$error && !this.v$.$invalid) {
          axios.post(variables.MONGOAPI+"Client",{
